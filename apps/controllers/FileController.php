...
class FileController extends Zend_Controller_Action
{
	...
	public function receiveAction()
	{
		...
		// デフォルトの保存先を設定する
		$upload->setDestination(STAGING_ROOT_PATH . '/file/');
		...
		foreach ($files as $file => $info) {
			...
			// ディレクトリを決定
			if (!$cateId) {
				$directory = STAGING_ROOT_PATH . '/file/';
			} else {
				// カテゴリテーブルから取得
		...
		// ファイルサイズを取得
		$fsize = filesize($directory . $names);
    ...
    /**
     * ファイルサイズをBBnavi用に編集して返す(128338 -> 128KB など)
     * 
     * @param  int    $size ファイルサイズ
     * @return string       フォーマット済みのファイルサイズ
     */
